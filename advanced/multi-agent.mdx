---
title: "Multi-Agent RL: Cooperative Combat with Independent Learners"
sidebarTitle: "Multi-Agent"
description: "Train a team of 5 agents in a custom grid-world Combat environment using independent DQN learners, where agents coordinate to defeat an opposing bot team."
---
...
The simplest MARL approach is **independent learning**: each agent maintains its own policy and replay buffer and is trained as if the other agents were part of the (non-stationary) environment. Despite its simplicity, independent learning is effective in many cooperative settings because agents implicitly coordinate through shared rewards.

---

## The Combat Environment

The `Combat` class (from `combat.py`) simulates a 15×15 grid-world battle between two teams of 5 agents. Each agent can move in four directions, do nothing, or attack one of the 5 opponents. All agents start with 3 health points. The game ends when one team is eliminated or 100 steps have elapsed.
...
# Sample random actions for all 5 agents
actions = [env.action_space[i].sample() for i in range(5)]
obs, rewards, dones, info = env.step(actions)
```

<Tip>
The `action_space` for each agent is `Discrete(5 + n_opponents)`: actions 0–3 are directional moves, action 4 is no-op, and actions 5–9 are attacks targeting opponents 0–4.
</Tip>
...
| `action_space` | `MultiAgentActionSpace` — list of `Discrete(10)` spaces |
...
A single PPO network is shared across both agents in the simplified 2-agent version of the notebook. Each agent's observation (150 floats) is fed independently to the same network:
...
class PPO:
    def __init__(self):
        self.model_action = torch.nn.Sequential(
            ...
            torch.nn.Linear(64, 7),   # 5 moves + 2 attack targets (2-agent version)
