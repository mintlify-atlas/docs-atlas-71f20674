---
title: "Actor-Critic: Combined Policy and Value Learning"
sidebarTitle: "Actor-Critic"
description: "Implement the Actor-Critic algorithm, which trains a policy network (actor) and a value network (critic) simultaneously to reduce variance in policy gradient updates."
---
...
Actor-Critic collects a full episode before performing an update, similar to REINFORCE but now also recording the next state so the critic can compute bootstrap targets.
...
def get_data():
    ...
    # Convert lists to tensors  [b, 4], [b, 1], [b, 1], [b, 4], [b, 1]
...
def train():
    ...
    for i in range(1000):
        ...
        # ── Critic ───────────────────────────────────────────────────
        # Current value estimates:  [b, 4] → [b, 1]
        values = model_td(states)

        # Bootstrap targets: V(s') · γ · (1 - done) + r
        ...
        if i % 50 == 0:
            print(i, sum([test(play=False) for _ in range(20)]) / 20)
