---
title: "REINFORCE: Policy Gradient for Discrete Actions"
...
Unlike epsilon-greedy, the policy is sampled stochastically from the probability distribution output by the network.
...
<Tip>
The high variance of REINFORCE is its main weakness. Actor-Critic methods (see the next page) replace `G_t` with an **advantage estimate** `A_t = G_t - V(s_t)` to reduce variance at the cost of introducing a learned baseline.
</Tip>
...
<Tabs>
  <Tab title="Policy Gradient (REINFORCE)">
    ...
    - **Stochastic policy**: outputs a probability distribution; action is sampled.
